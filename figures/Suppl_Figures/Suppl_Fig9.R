...
data <- df_contaminantFree_sumGenus_maxAbundance_sepsisGenera %>% 
        mutate(tmp_col = ifelse(sum_rel_abundance > max_Spos & 
                               sum_count_abundance >= min_hard_count_genus, yes = "S+", 
...
co_elevation_all_reverse <- rbind(data, metadata) %>% 
    mutate(tmp_col = factor(tmp_col, levels = c("S+", "none", "n/a, 'low abundance'"))) %>%
        ggplot(aes(x = FID, y = variable)) +
            ...
            theme_bw() + labs(col = "Abundance exceeds", shape = "Abundance exceeds", size = "Relative abundance,\n-log10", 
                            x = "", y = "") + 
            scale_color_manual(values = c("S+" = "#6066B6",  
                                          "n/a, 'low abundance'" = "black",
                                          "none" = "black")) +
            scale_shape_manual(values = c("S+" = 18, 
                                          "n/a, 'low abundance'" = 1,
                                          "none" = 3)) +
...
ggsave(paste0("../../output_figures/SupplFig9_co-elevation_all_reverse.png"), plot = co_elevation_all_reverse, 
           width = 28, height = 14, units = "cm")
ggsave(paste0("../../output_figures/SupplFig9_co-elevation_all_reverse.pdf"), plot = co_elevation_all_reverse, 
           width = 28, height = 14, units = "cm")
